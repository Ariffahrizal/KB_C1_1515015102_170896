...
5. Subgoal berikutnya yaitu cek_bulan_ulang_tahun(Bulan_ini, Tanggal). 
   ...
   hasil cocok lalu dilakukan pencocokkan kembali data menggunakan lacakbalik dimulai dari subgoal berikutnya 
   yaitu daftar_telepon(Orang, _, Tanggal) dengan unifikasi maka data teratas berikutnya akan dibaca.

6. Di subgoal terakhir ada cetak_orang(Orang) subgoal ini hanya digunakan untuk mencetak hasil dari subgoal diatas 
   Terdapat predikat fail pada subgoal yang terakhir yang berarti memaksa lacak balik meski pun telah mendapatkan solusi. 
